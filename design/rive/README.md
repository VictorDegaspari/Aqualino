...
Validação da integração: TypeScript e lint aprovados; 16 testes do onboarding aprovados; bundle Android gerado com o `.riv` em `raw/`. O arquivo exportado também foi carregado no runtime web oficial: artboard e máquina de estados encontrados, quadros diferentes durante a reprodução e fundo transparente. A suíte completa teve 322 testes aprovados e uma falha não relacionada em `GroupChallengePanel.test.tsx` (expectativa pelo texto “15 minutos”). Em 2026-09-10, a compilação nativa e a reprodução dos seis arquivos `.riv` foram validadas no Samsung Galaxy A30s após corrigir a exigência indevida de ViewModel no runtime Android; veja os [detalhes da correção e validação](../../apps/mobile/src/assets/mascot/rive/README.md).
