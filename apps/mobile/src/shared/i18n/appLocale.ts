export type AppLocale = 'pt-BR' | 'en-US' | 'es-ES';

export const localeOptions: ReadonlyArray<{
  value: AppLocale;
  flag: string;
  label: string;
  country: string;
}> = [
  {value: 'pt-BR', flag: '🇧🇷', label: 'Português', country: 'Brasil'},
  {value: 'en-US', flag: '🇺🇸', label: 'English', country: 'United States'},
  {value: 'es-ES', flag: '🇪🇸', label: 'Español', country: 'España'},
];

export function normalizeAppLocale(value: string | undefined | null, fallback: AppLocale = 'pt-BR'): AppLocale {
  switch (value?.trim().toLowerCase().split(/[-_]/)[0]) {
    case 'en': return 'en-US';
    case 'es': return 'es-ES';
    case 'pt': return 'pt-BR';
    default: return fallback;
  }
}

export const appCopy = {
  'es-ES': {
    welcome: {
      eyebrow: 'TU COMPAÑERO DE HIDRATACIÓN',
      title: 'Vamos a cuidarte, una gota a la vez.',
      subtitle: 'Crea un hábito saludable con Aqualino a tu lado.',
      languageTitle: 'Elige el idioma de la app',
      languageSubtitle: 'Podrás cambiarlo después en las preferencias.',
      goalTitle: '¿Cuál es tu meta diaria?',
      goalSubtitle: 'Aqualino acompañará cada sorbo hasta alcanzarla.',
      goalInputLabel: 'Meta diaria en mililitros',
      accountTitle: '¡Ya casi está!',
      accountSubtitle: 'Crea tu cuenta para guardar tu progreso.',
      continue: 'Continuar', back: 'Volver', step: 'Paso', of: 'de',
      createAccount: 'Crear mi cuenta', alreadyHaveAccount: 'Ya tengo una cuenta',
      returningTitle: 'Qué bueno tenerte de vuelta.',
      returningSubtitle: 'Inicia sesión de nuevo o prepara Aqualino para una cuenta nueva.',
      savedAccounts: 'Tus cuentas', continueAs: 'Continuar como', signIn: 'Iniciar sesión', signingIn: 'Iniciando sesión',
      manageAccounts: 'Administrar cuentas',
      manageAccountsSubtitle: 'Elimina una cuenta de este dispositivo cuando ya no quieras acceso rápido.',
      removeAccount: 'Eliminar cuenta', backToAccounts: 'Volver a las cuentas',
      resumeAccountError: 'No se pudo retomar esta cuenta en este momento.',
      removeAccountError: 'No se pudo eliminar esta cuenta en este momento.',
      offlineAccountError: 'No tienes conexión a internet. Comprueba tu conexión e inténtalo de nuevo.',
      addAccount: 'Añadir una cuenta nueva', addAccountSubtitle: 'Reiniciar idioma, meta y creación de la cuenta.',
      savedAccountSecurity: 'Las cuentas guardadas usan una credencial protegida del dispositivo. Elimina una cuenta para cancelar el acceso rápido.',
    },
    auth: {
      loginEyebrow: 'QUÉ BUENO VERTE', loginTitle: 'Entra en tu océano.',
      loginSubtitle: 'Tu ritmo de hidratación te está esperando.',
      registerEyebrow: 'EMPIEZA A TU RITMO', registerTitle: 'Crea tu cuenta.',
      registerSubtitle: 'Aqualino solo necesita un momento para conocerte.',
      email: 'Correo electrónico', password: 'Contraseña', displayName: '¿Cómo quieres que te llamemos?',
      username: 'Nombre de usuario', usernamePlaceholder: 'ana_azul',
      checkingUsername: 'Comprobando este @…', usernameAvailable: 'Este @ está disponible.',
      usernameUnavailable: 'Este @ no está disponible.', passwordPlaceholder: '8+ caracteres, letras y números',
      signIn: 'Iniciar sesión', createAccount: 'Crear mi cuenta', noAccount: 'Todavía no tengo una cuenta',
      terms: 'He leído y acepto los Términos y la Política de Privacidad.', language: 'Idioma',
      loginError: 'No se pudo iniciar sesión.', registerError: 'No se pudo crear la cuenta.',
    },
    setup: {
      title: 'Prepara tu Aqualino', subtitle: 'Define tu ritmo. Podrás ajustarlo todo después.',
      goalTitle: 'Tu meta diaria', goalSubtitle: 'Una meta es un apoyo para crear el hábito.',
      goalMascotLabel: 'Aqualino está emocionado por acompañar tu meta',
      quickVolumes: 'Cantidades rápidas', quickVolumesSubtitle: 'Elige los vasos y botellas que más usas.',
      language: 'Idioma de la app', languageSubtitle: 'Elige cómo prefieres usar Aqualino.', timezone: 'Zona horaria',
      notice: 'Si tienes una restricción de líquidos, sigue las indicaciones de tu profesional de salud.',
      finish: 'Terminar y empezar', saveError: 'No se pudieron guardar tus preferencias.',
      savingTitle: 'Preparando tu océano', savingSubtitle: 'Estamos guardando tu meta para que Aqualino empiece a acompañarte.',
      retry: 'Intentar de nuevo',
    },
    goalGauge: {
      eyebrow: 'TU META DIARIA', status: 'Meta elegida', comparison: 'Aqualino celebrará cada sorbo contigo.',
      hint: 'El agua acompaña la meta elegida',
    },
  },
  'pt-BR': {
    welcome: {
      eyebrow: 'SEU COMPANHEIRO DE HIDRATAÇÃO',
      title: 'Vamos cuidar de você, uma gota por vez.',
      subtitle: 'Crie um hábito leve com o Aqualino ao seu lado.',
      languageTitle: 'Escolha o idioma do app',
      languageSubtitle: 'Você poderá mudar isso nas preferências depois.',
      goalTitle: 'Qual é a sua meta diária?',
      goalSubtitle: 'O Aqualino vai acompanhar cada gole até ela.',
      goalInputLabel: 'Meta diária em mililitros',
      accountTitle: 'Quase lá!',
      accountSubtitle: 'Crie sua conta para salvar seu progresso.',
      continue: 'Continuar',
      back: 'Voltar',
      step: 'Etapa',
      of: 'de',
      createAccount: 'Criar minha conta',
      alreadyHaveAccount: 'Já tenho uma conta',
      returningTitle: 'Que bom ter você de volta.',
      returningSubtitle: 'Entre novamente ou prepare o Aqualino para uma nova conta.',
      savedAccounts: 'Suas contas',
      continueAs: 'Continuar como',
      signIn: 'Entrar',
      signingIn: 'Entrando',
      manageAccounts: 'Gerenciar contas',
      manageAccountsSubtitle: 'Remova uma conta deste aparelho quando não quiser mais acesso rápido.',
      removeAccount: 'Remover conta',
      backToAccounts: 'Voltar para contas',
      resumeAccountError: 'Não foi possível retomar esta conta agora.',
      removeAccountError: 'Não foi possível remover esta conta agora.',
      offlineAccountError: 'Você não está conectado à internet. Verifique sua conexão e tente novamente.',
      addAccount: 'Adicionar nova conta',
      addAccountSubtitle: 'Reiniciar idioma, meta e criação da conta.',
      savedAccountSecurity: 'As contas salvas usam uma credencial protegida do aparelho. Remova a conta para encerrar esse acesso rápido.',
    },
    auth: {
      loginEyebrow: 'QUE BOM VER VOCÊ',
      loginTitle: 'Entre no seu oceano.',
      loginSubtitle: 'Seu ritmo de hidratação está esperando por você.',
      registerEyebrow: 'COMECE PELO SEU RITMO',
      registerTitle: 'Crie sua conta.',
      registerSubtitle: 'Leva só um instante para o Aqualino te conhecer.',
      email: 'E-mail',
      password: 'Senha',
      displayName: 'Como quer ser chamado?',
      username: 'Nome de usuário',
      usernamePlaceholder: 'ana_azul',
      checkingUsername: 'Verificando este @…',
      usernameAvailable: 'Este @ está disponível.',
      usernameUnavailable: 'Este @ já está indisponível.',
      passwordPlaceholder: '8+ caracteres, letras e números',
      signIn: 'Entrar',
      createAccount: 'Criar minha conta',
      noAccount: 'Ainda não tenho conta',
      terms: 'Li e aceito os Termos e a Política de Privacidade.',
      language: 'Idioma',
      loginError: 'Não foi possível entrar.',
      registerError: 'Não foi possível criar a conta.',
    },
    setup: {
      title: 'Prepare seu Aqualino',
      subtitle: 'Defina seu ritmo. Você poderá ajustar tudo depois.',
      goalTitle: 'Sua meta diária',
      goalSubtitle: 'Uma meta é só um apoio para criar o hábito.',
      goalMascotLabel: 'Aqualino está animado para acompanhar sua meta',
      quickVolumes: 'Volumes rápidos',
      quickVolumesSubtitle: 'Escolha seus copos e garrafas mais usados.',
      language: 'Idioma do app',
      languageSubtitle: 'Escolha como prefere usar o Aqualino.',
      timezone: 'Fuso',
      notice: 'Se você possui restrição hídrica, siga a orientação do seu profissional de saúde.',
      finish: 'Concluir e começar',
      saveError: 'Não foi possível salvar suas preferências.',
      savingTitle: 'Preparando seu oceano',
      savingSubtitle: 'Estamos salvando sua meta para o Aqualino começar a te acompanhar.',
      retry: 'Tentar novamente',
    },
    goalGauge: {
      eyebrow: 'SUA META DIÁRIA',
      status: 'Meta escolhida',
      comparison: 'O Aqualino vai celebrar cada gole com você.',
      hint: 'A água acompanha a meta escolhida',
    },
  },
  'en-US': {
    welcome: {
      eyebrow: 'YOUR HYDRATION COMPANION',
      title: 'Let’s take care of you, one drop at a time.',
      subtitle: 'Build a gentle habit with Aqualino by your side.',
      languageTitle: 'Choose the app language',
      languageSubtitle: 'You can change this later in preferences.',
      goalTitle: 'What is your daily goal?',
      goalSubtitle: 'Aqualino will follow every sip until you reach it.',
      goalInputLabel: 'Daily goal in milliliters',
      accountTitle: 'Almost there!',
      accountSubtitle: 'Create your account to save your progress.',
      continue: 'Continue',
      back: 'Back',
      step: 'Step',
      of: 'of',
      createAccount: 'Create my account',
      alreadyHaveAccount: 'I already have an account',
      returningTitle: 'It’s good to have you back.',
      returningSubtitle: 'Sign in again or prepare Aqualino for a new account.',
      savedAccounts: 'Your accounts',
      continueAs: 'Continue as',
      signIn: 'Sign in',
      signingIn: 'Signing in',
      manageAccounts: 'Manage accounts',
      manageAccountsSubtitle: 'Remove an account from this device when you no longer want quick access.',
      removeAccount: 'Remove account',
      backToAccounts: 'Back to accounts',
      resumeAccountError: 'We could not resume this account right now.',
      removeAccountError: 'We could not remove this account right now.',
      offlineAccountError: 'You are not connected to the internet. Check your connection and try again.',
      addAccount: 'Add a new account',
      addAccountSubtitle: 'Restart language, goal, and account creation.',
      savedAccountSecurity: 'Saved accounts use a protected device credential. Remove an account to end quick access.',
    },
    auth: {
      loginEyebrow: 'NICE TO SEE YOU',
      loginTitle: 'Enter your ocean.',
      loginSubtitle: 'Your hydration rhythm is waiting for you.',
      registerEyebrow: 'START WITH YOUR RHYTHM',
      registerTitle: 'Create your account.',
      registerSubtitle: 'It only takes a moment for Aqualino to get to know you.',
      email: 'Email',
      password: 'Password',
      displayName: 'What should we call you?',
      username: 'Username',
      usernamePlaceholder: 'ana_blue',
      checkingUsername: 'Checking this @…',
      usernameAvailable: 'This @ is available.',
      usernameUnavailable: 'This @ is unavailable.',
      passwordPlaceholder: '8+ characters, letters and numbers',
      signIn: 'Sign in',
      createAccount: 'Create my account',
      noAccount: 'I do not have an account yet',
      terms: 'I have read and accept the Terms and Privacy Policy.',
      language: 'Language',
      loginError: 'We could not sign you in.',
      registerError: 'We could not create your account.',
    },
    setup: {
      title: 'Set up your Aqualino',
      subtitle: 'Set your rhythm. You can adjust everything later.',
      goalTitle: 'Your daily goal',
      goalSubtitle: 'A goal is just support for building your habit.',
      goalMascotLabel: 'Aqualino is excited to follow your goal',
      quickVolumes: 'Quick amounts',
      quickVolumesSubtitle: 'Choose your most-used cups and bottles.',
      language: 'App language',
      languageSubtitle: 'Choose how you prefer to use Aqualino.',
      timezone: 'Time zone',
      notice: 'If you have a fluid restriction, follow your healthcare professional’s guidance.',
      finish: 'Finish and start',
      saveError: 'We could not save your preferences.',
      savingTitle: 'Preparing your ocean',
      savingSubtitle: 'We are saving your goal so Aqualino can start following along.',
      retry: 'Try again',
    },
    goalGauge: {
      eyebrow: 'YOUR DAILY GOAL',
      status: 'Goal selected',
      comparison: 'Aqualino will celebrate every sip with you.',
      hint: 'The water follows your selected goal',
    },
  },
} as const;
