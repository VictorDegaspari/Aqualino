...
export function GroupChallengePanel({challenge, result, rules, offline = false, locale = 'pt-BR'}: {
  ...
  return <View style={styles.container}>
    ...
    <View style={styles.links}>
      {currentRules ? <Pressable accessibilityRole="button" accessibilityLabel={locale === 'es-ES' ? "Reglas y premios" : english ? 'Rules and rewards' : 'Regras e prêmios'} onPress={() => open('rules')} style={({pressed}) => [styles.link, pressed && styles.linkPressed]}>
        <AqualinoIcon name="medalGold" size={20} />
        <Text style={styles.linkText}>{locale === 'es-ES' ? "Reglas y premios" : english ? 'Rules and rewards' : 'Regras e prêmios'}</Text>
      </Pressable> : null}
      {result && result.id !== challenge?.id ? <Pressable accessibilityRole="button" onPress={() => open('result')} style={({pressed}) => [styles.link, pressed && styles.linkPressed]}>
        <Text style={styles.linkText}>{result.status === 'settling' ? (locale === 'es-ES' ? "Calculando la ronda anterior" : english ? 'Finalizing previous round' : 'Apurando rodada anterior') : result.reward ? (locale === 'es-ES' ? "Tu premio de la ronda anterior" : english ? 'Your previous reward' : 'Seu prêmio da rodada anterior') : (locale === 'es-ES' ? "Resultado anterior" : english ? 'Previous results' : 'Resultado anterior')}</Text>
      </Pressable> : null}
    </View>
    {offline && challenge?.leaderboard?.length ? <Text style={styles.notice}>{locale === 'es-ES' ? "Última clasificación sincronizada. Los registros pendientes cuentan tras su confirmación." : english ? 'Last synced standings. Pending logs count after confirmation.' : 'Último placar sincronizado. Registros pendentes entram após confirmação.'}</Text> : null}
...
function rulesMessage(rules: GroupChallengeRules, locale: AppLocale): string {
  if (locale === 'es-ES') {
    ...
    return `De 2 a 5 personas, durante 7 días en la zona horaria del grupo.\n\nTu meta diaria queda fija al inicio. Cada día vale hasta ${rules.daily_points_cap} puntos y la ronda hasta ${rules.total_points_cap}. Beber por encima de la meta no da puntos extra.\n\nSe mantienen los empates: 1.º, 1.º, 3.º. Se comparan los puntos con ${rules.points_decimals} decimales. Los jugadores empatados comparten la misma medalla. Hay que sumar puntos para recibir una medalla o premio.\n\nCada primer puesto recibe un sorteo gratuito e independiente:\n${rewards}\n\nSe aceptan sincronizaciones hasta ${rules.sync_grace_minutes} minutos después del final. El resultado y los premios son definitivos cuando se cierran todas las votaciones de fotos (hasta 12 horas después de cada envío).\n\nQuien se une después del inicio participa en la próxima ronda. Empieza a medianoche después del séptimo día si quedan al menos 2 miembros. El XP personal, el plan Pro y las pociones no cambian la clasificación.`;
  }
  if (locale === 'en-US') {
    ...
    return `2–5 people, for 7 days in the group timezone.\n\nYour daily goal is fixed at the start. Each day earns up to ${rules.daily_points_cap} points, with ${rules.total_points_cap} per round. Water above your goal adds no extra points.\n\nTies are kept: 1st, 1st, 3rd. Scores use ${rules.points_decimals} decimal places. Tied players share the same medal. You must score points to earn a medal or reward.\n\nEvery 1st place gets one free, independent draw:\n${rewards}\n\nLogs may sync for ${rules.sync_grace_minutes} minutes after the round ends. Results and rewards become final once all photo votes close (up to 12 hours after a submission).\n\nLate arrivals join the next round. It starts at midnight after day 7 if at least 2 members remain. Personal XP, Pro and potions do not change the standings.`;
  }
  ...
  return `De 2 a 5 pessoas, durante 7 dias no fuso do grupo.\n\nSua meta fica fixa no início. Cada dia vale até ${rules.daily_points_cap} pontos e a rodada até ${rules.total_points_cap}. Beber além da meta não dá pontos extras.\n\nEmpates são mantidos: 1º, 1º, 3º. Pontos são comparados com ${rules.points_decimals} casas decimais. Ouro, prata e bronze seguem a posição, inclusive nos empates. É preciso pontuar para receber medalha ou prêmio.\n\nCada 1º lugar recebe um sorteio gratuito e independente:\n${rewards}\n\nO placar aceita sincronizações por ${rules.sync_grace_minutes} minutos após o fim. Resultado e prêmios ficam definitivos após encerrar também as votações de fotos (até 12 horas após cada envio).\n\nQuem entra após o início participa da próxima rodada. A próxima começa à meia-noite após o sétimo dia, se houver pelo menos 2 integrantes. XP pessoal, plano Pro e poções não mudam o placar.`;
}
...
const styles = StyleSheet.create({
  ...
  link: {minHeight: 44, flexGrow: 1, flexShrink: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: challengeTheme.colors.borderStrong, backgroundColor: challengeTheme.colors.panelSoft},
  linkPressed: {opacity: 0.8},
  linkText: {flexShrink: 1, fontSize: 12, lineHeight: 17, color: challengeTheme.colors.cyanStrong, fontWeight: '700', textAlign: 'center'},
