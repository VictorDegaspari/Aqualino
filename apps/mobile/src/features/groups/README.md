...
Com um grupo ativo, a tela começa pelo nome e pela lista de participantes, com avatar, nível e acesso ao perfil. As vagas livres aparecem em uma única linha, apenas enquanto a entrada estiver aberta. A engrenagem usa o botão padrão com efeito de pressão e abre apenas configurações e fuso do grupo. “Convidar” abre um painel exclusivo para código, compartilhamento e renovação; apenas o responsável gerencia o convite. A saída fica no fim da tela; renovar convite e sair mantêm suas confirmações. Placar e votações vêm depois dos participantes.
...
O contrato está em `packages/contracts/openapi.yaml`. O responsável inicia a primeira rodada pela Home, na aba Grupo, com pelo menos duas pessoas. A Home e a aba Equipes exibem o placar real, as regras e o último resultado. A entrada no grupo fecha no início da primeira rodada, inclusive para convites consultados anteriormente. O placar usa a mesma referência diária de 2.000 ml para todos, com teto de 100 pontos; metas pessoais não alteram a pontuação. Rodadas ainda não finalizadas adotam essa referência ao atualizar, e resultados já finalizados são preservados. O elenco fica fixo na largada; os pontos são comparados em centésimos e os empates usam posições como 1º, 1º, 3º. A rodada tem sete dias civis no fuso do grupo e 15 minutos finais para sincronização, e aguarda também o encerramento das votações abertas antes da confirmação das medalhas e dos sorteios dos primeiros colocados (70% XP, 20% congelamento, 10% reacender). Por padrão, o responsável inicia cada próxima rodada manualmente para a meia-noite seguinte, com pelo menos duas pessoas. O botão de configurações do grupo reúne votação de fotos e reinício automático; somente o líder altera essas opções. Ao habilitar `auto_restart`, as rodadas seguintes começam automaticamente ao final da janela. Desabilitar não cancela uma rodada já iniciada ou agendada.
