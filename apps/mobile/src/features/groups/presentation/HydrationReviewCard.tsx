  ...
  return (
    ...
    >
      ...
      {photoError ? <View style={styles.photoState}>
        ...
        <RaisedButton
          accessibilityLabel={locale === 'es-ES' ? 'Volver a cargar la foto' : english ? 'Reload photo' : 'Recarregar foto'}
          onPress={() => {setPhotoError(false); setPhotoAttempt(value => value + 1);}}
          label={locale === 'es-ES' ? 'Reintentar' : english ? 'Retry' : 'Tentar novamente'}
          variant="outlined"
          tone="aqua"
          size="compact"
        />
      ...
      <View style={styles.actions}>
        <RaisedButton
          testID="review-invalid"
          accessibilityLabel={locale === 'es-ES' ? 'Marcar como inválido' : english ? 'Mark as invalid' : 'Marcar como inválida'}
          disabled={!canDecide}
          onPress={() => {submit('invalid');}}
          label="×"
          variant="outlined"
          tone="danger"
        />
        ...
        <RaisedButton
          testID="review-valid"
          accessibilityLabel={locale === 'es-ES' ? 'Marcar como válido' : english ? 'Mark as valid' : 'Marcar como válida'}
          disabled={!canDecide}
          onPress={() => {submit('valid');}}
          label="✓"
          tone="success"
        />
      </View>
